...
for (cluster_id in clusters) {
  ...
  unfilter_output_file <- file.path(output_folder, paste0(Project_Name,"_cluster_", cluster_id, "_unfilt_gene_expression.txt"))
  upreg_output_file <- file.path(output_folder, paste0(Project_Name,"_cluster_", cluster_id, "_upreg_gene_expression.txt"))
  dnreg_output_file <- file.path(output_folder, paste0(Project_Name,"_cluster_", cluster_id, "_dnreg_gene_expression.txt"))
...
xlsx_output_file <- file.path(output_folder, paste0(Project_Name,"_SeuratCluster_DEG.xlsx"))
...
if (all(c(!is.na(Marker_Column),!is.null(Marker_Column)),Marker_Column!="")) {
  ...
  for(cells in cell_types){
    ...
    # write the marker genes to a separate file 
    unfilter_output_file <- file.path(output_folder, gsub("[[:punct:]]",".",gsub(" ","_",paste0(Project_Name,"_",cells, "_unfilt_gene_expression.txt"))))
    upreg_output_file <- file.path(output_folder, gsub("[[:punct:]]",".",gsub(" ","_",paste0(Project_Name,"_",cells, "_upreg_gene_expression.txt"))))
    dnreg_output_file <- file.path(output_folder, gsub("[[:punct:]]",".",gsub(" ","_",paste0(Project_Name,"_",cells, "_dnreg_gene_expression.txt"))))
  ...
  # Save the workbook to an Excel file
  xlsx_output_cell_file <- file.path(output_folder, paste0(Project_Name,"_",Marker_Column,"_DEG.xlsx"))
  saveWorkbook(wb2, xlsx_output_cell_file)
  
  # Find markers for each cluster with a minimum percentage threshold
  all.markers_choice <- FindAllMarkers(seurat_obj, min.pct = 0.1)
  
  # Calculate the difference between pct.1 and pct.2 and add a new column
  all.markers_choice$diff_pct <- all.markers_choice$pct.1 - all.markers_choice$pct.2
  
  # Specify the file path for saving the all markers data
  all_marker_choice_file_path <- file.path(output_folder, paste0(Project_Name,"_",Marker_Column,"_all_markers.txt"))
  
  all.markers_choice <- all.markers_choice %>% relocate("gene")
  
  # Write the all markers data to a tab-separated file
  write.table(all.markers_choice, file = all_marker_choice_file_path, 
...
for (i in names(subset_seurat_obj@assays)) {
  ...
  write.table(scaled_counts, file = paste0(output_folder, "/", Project_Name,"_",num_cells,"_",i,"_ScaledCounts.txt"),
              sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
  write.table(raw_counts, file = paste0(output_folder, "/", Project_Name,"_",num_cells,"_",i,"_raw_counts.txt"),
              sep = "\t", row.names = FALSE, col.names = TRUE)
  write.table(normalized_data, file = paste0(output_folder, "/", Project_Name,"_",num_cells,"_",i,"_normalized_counts.txt"),
              sep = "\t", row.names = FALSE, col.names = TRUE)
  write.table(metadata, file = paste0(output_folder, "/", Project_Name,"_",num_cells,"_",i,"_metafile.txt"),
              sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
}
...
# Specify the file path for saving the all markers data
all_marker_file_path <- file.path(output_folder, paste0(Project_Name,"_SeuratClusters_all_markers.txt"))
